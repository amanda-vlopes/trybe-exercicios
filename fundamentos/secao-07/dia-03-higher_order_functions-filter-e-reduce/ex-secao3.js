...
// Tentativa usando duas HOFs e sem juntar os nomes antes de dividir 

// const countA = () => {
//   const letrasArray = names.map((nome) => nome.toLowerCase().split(''));
//   // letrasArray.reduce((acc, curr, index) => curr[index] === 'a' ? acc + 1 : acc + 0, 0);
//   return letrasArray.forEach((letras) => letras.reduce((acc, curr) => curr === 'a' ? acc + 1 : acc + 0, 0))
// }

const countA = () => {
  const letrasArray = names.join('').toLowerCase().split('');
  return letrasArray.reduce((acc, curr) => curr === 'a' ? acc + 1 : acc + 0, 0);
}
...
const studentAverage = () => {
  const medias =  grades.map((notas) => notas.reduce((acc, curr) => acc + curr) / notas.length);
  return students.map((estudante, index) => ({ name: estudante, average: medias[index] }));
}
